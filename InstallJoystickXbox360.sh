...
sleep 3
clear
configuracao1="SETAS PARA ESCOLHER E ENTER PARA CONFIRMAR"
cont="$[${#configuracao1} + 4]"
opcao=$(dialog --title "MENU" --menu "$configuracao1" 10 $cont 3 \
"1" "PARA INSTALAR" \
"2" "PARA REMOVER" \
"3" "PARA SAIR" \
--stdout)
...
case $opcao in
	1)
	configuracao1="PARA CONFIGURAÇÃO COM ANALÓGICO ESQUERDO COM SENTIDO INVERTIDO"
	cont="$[${#configuracao1} + 10]"
...
EOF
		;;
		*)
		configuracao1="Configuração cancelada..."
		cont="$[${#configuracao1} + 4]"
		dialog --infobox "$configuracao1" 3 $cont
		sleep 3
		clear
...
opção escolhida agora - Opção $xbox." 6 65
	...
	;; 
	*)
	configuracao1="Instalação cancelada..."
	cont="$[${#configuracao1} + 4]"
	dialog --infobox "$configuracao1" 3 $cont
	sleep 3
	clear
